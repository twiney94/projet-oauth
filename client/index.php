...
define("FB_CLIENT_ID", '4336290183137571');
define("CLIENT_SECRET", '04054cf433eeb3976252c81b6d657fda');
define("FB_CLIENT_SECRET", '0c5101296d2b204d4c7d770e544e8211');
...
function login()
{
    ...
    $paypalQueryParams = http_build_query([
        "state"=>bin2hex(random_bytes(16)),
        "client_id"=> PAYPAL_CLIENT_ID,
        "scope"=>"openid",
        "redirect_uri"=>"https://localhost/paypal_oauth_success",
    ]);
    ...
    echo "<a href=\"https://api.sandbox.paypal.com/v1/oauth2/token?grant_type=authorization_code&code={$paypalQueryParams}\">Login with PayPal</a>";
}
...
function getTokenPaypal()
{
    ...
    $queryParams = http_build_query([
        ...
        "redirect_uri"=>"https://api-m.sandbox.paypal.com/v1/oauth2/token",
        "code"=> $code,
        "grant_type"=>"authorization_code",
    ]);

    $url = "https://api-m.sandbox.paypal.com/v1/oauth2/token" . "?{$queryParams}";
    $response = file_get_contents($url);
    if (!$response) {
        echo $http_response_header;
        return;
